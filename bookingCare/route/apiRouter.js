...
router.get('/allspecial', async function (req,res) {
    ...
    let { rows } = await pool.query("SELECT specialization_id, specialization_name, specialization_staff, related_disease, image, doctor_id FROM specialization");
...
router.get('/hospital', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from hospitalinfo;");
...
router.get('/doctor/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from doctor inner join confirmbooking on doctor.doctor_id = confirmbooking.doctor_id where doctor.doctor_id = $1",[id])
...
router.get('/onlydoctor/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from doctor where doctor_id = $1",[id])
...
router.get('/hospital/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from hospitalinfo where hospital_id = $1",[id])
...
router.get('/special/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from specialization where specialization_id = $1",[id]);
    if (!rows) {
        res.status(404).json({error : "error"})
    } else {
        res.status(200).json(rows);
...
router.get('/patient/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("SELECT * FROM confirmbooking inner join doctor on confirmbooking.doctor_id = doctor.doctor_id inner join patient on confirmbooking.patient_id = patient.patient_id where patient.patient_id = $1",[id])
...
router.get('/allpatient', async function (req,res) {
    ...
    let { rows } = await pool.query("Select patient_id,password from patient");
...
router.get('/hospitalfromdoctor/:id', async function (req,res) {
    ...
    let { rows } = await pool.query("Select * from doctor inner join hospitalinfo on doctor.hospital_id = hospitalinfo.hospital_id where doctor.doctor_id = $1",[id])
