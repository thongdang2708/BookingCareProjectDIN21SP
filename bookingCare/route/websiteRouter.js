...
router.post('/post/register', async function (req,res) {
    ...
    let { rows } = await pool.query("Select username from patient where username = $1 or email = $2",[username,email]);
    ...
    if (rows.length) {
        ...
        if (passwordCheck.length < 6) {
            ...
            if (err) {
                ...
                console.log(hash);
                pool.query(`INSERT INTO patient (username,hashedpassword,first_name_patient,last_name_patient,location,phone_number,email) values ('${username}','${hash}', '${firstname}','${lastname}','${location}','${phone}','${email}')`, (err,result) => {
                    if (err) {
                        console.log('error');
                    } else {
...
router.post('/login', async function (req,res) {
    ...
    let {  username, password } = req.body;
        ...
        let { rows } = await pool.query("Select * from patient where username = $1",[username]);
...
router.post('/submitformforpatient' , function (req,res) {
    ...
    pool.query("Insert into confirmbooking (doctor_id,patient_id,description,date,month,year,start_time,end_time) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)", [doctor_id,patient_id,description,date_el,month_el,year_el,starttime,endtime], (err,result) => {
...
router.post('/submitform' , function (req,res) {
    ...
    pool.query("Insert into confirmbooking (doctor_id,description,date,month,year,start_time,end_time) VALUES ($1,$2,$3,$4,$5,$6,$7)", [doctor_id,description,date_el,month_el,year_el,starttime,endtime], (err,result) => {
